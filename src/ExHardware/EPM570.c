...
__IO int32_t TimeoutCnt;		//Counter for timeout, value in ms
__IO FlagStatus SRAM_TimeoutState = RESET;
__IO State_TypeDef wrSRAM = STOP;       // флаг сигнализирующий о записи в память
...
static void SRAM_TimeoutCounterSetState(FunctionalState NewState);
...
{   
	...
	if(pnt_gOSC_MODE->oscSync != Sync_NONE)
	{
		if(HostMode == DISABLE)
		{
			regVal = (uint32_t) ( (NumPoints - (tTrigPosition / (*SweepScale))) / InterliveCoeff);		 // значение для записи в регистры ПЛИС
			preTrigger_time = (tTrigPosition * Get_Decimation() * 10 * ActiveMode->oscNumPoints_Ratio) / (1000 * (*SweepScale));
		}
		else
		{
			regVal = (NumPoints - (trigPosX_cursor.Position - 1)) / InterliveCoeff;
...
{   
	...
	if(pnt_gOSC_MODE->oscSync != Sync_NONE)	reg_val += ((trigPosX_cursor.Position / ((*SweepScale) * InterliveCoeff)) - leftLimit);
...
* Return         : None
*******************************************************************************/
State_TypeDef Write_SRAM(void)
{   
	uint32_t TimeoutTime_ms;
	static uint16_t tColor = White, tFlag = 0, tCnt = 0;

	wrSRAM = RUN;         // Устанавливаем флаг сигнализирующий о записи в память
	NVIC_DisableIRQ(TIM2_IRQn);	   // запрет прерываний таймера опроса кнопок

	/* Interlive mode or not */
	if(gOSC_MODE.Interleave == TRUE){ EPM570_Write_Register(ExtPin_B, Enable_Interleave); GPIOA->BRR = GPIO_BRR_BR13; }
	else { EPM570_Write_Register(ExtPin_B, Disable_Interleave); GPIOA->BSRR = GPIO_BSRR_BS13; }

	/* Calculate needed timeout counter in ms */
	if(pnt_gOSC_MODE->oscSync == Sync_NONE) TimeoutTime_ms = 200;
	else TimeoutTime_ms = 200 + ((Get_numPoints() * Get_Decimation() * 10 * ActiveMode->oscNumPoints_Ratio) / (1000000 * (*SweepScale)) );
	...
	TimeoutCnt = TimeoutTime_ms;
	SRAM_TimeoutCounterSetState(ENABLE);
	...
	while((EPM570_Write_Ready == 0) && (wrSRAM != STOP))                                // Wait write end
	{
		...
		if(SRAM_TimeoutState == SET)
		{
			...
			if(pnt_gOSC_MODE->oscSync == Sync_AUTO){ EPM570_Sync(Sync_NONE); tColor = Yellow; break; }
			else tColor = White;

			/* Restart timeout timer(TIM7) */
			TimeoutCnt = TimeoutTime_ms;
			SRAM_TimeoutCounterSetState(ENABLE);
		}
	}
	...
	/* Запись закончилась, выключаем ОУ, АЦП, Генератор и сбрасываем сигналы старта записи внутри ПЛИС */
	Control_AD8129(DISABLE);						                           // Disable both AD8921
	...
	else if(pnt_gOSC_MODE->oscSync == Sync_AUTO) wrSRAM = COMPLETE;
	SRAM_TimeoutCounterSetState(DISABLE);

	NVIC_EnableIRQ(TIM2_IRQn);	  	// разрешение прерываний таймера опроса кнопок
	
	return wrSRAM;  				// Возвращаем флаг состояния записи в память
}


/*******************************************************************************
* Function Name  : SRAM_TimeoutCounterSetState
* Description    :
* Input          : None
* Return         : None
*******************************************************************************/
...
/*******************************************************************************
* Function Name  : SRAM_GetWriteState
* Description    :
* Input          : None
* Return         : None
*******************************************************************************/
...
{  
	...
	if((ActiveMode == &IntMIN_MAX) && (SamplesWindow > 776))
	{
		EPM570_RS_set; EPM570_RS_set;
		EPM570_RS_clr; EPM570_RS_clr;
	}
...
* Return         : None
*******************************************************************************/
void EPM570_Sync(SyncMode_TypeDef NewMode)
{
   if(NewMode != Sync_NONE)
   {
      EPM570_Write_Register(cnfPin, EPM570_cnfPin_DATA | Sync_ON);
      pnt_gOSC_MODE->oscPreHistory = 1;
   }
   else if(NewMode == Sync_NONE)
   {
	  EPM570_Write_Register(cnfPin, EPM570_cnfPin_DATA & ~Sync_ON);
